// Automatically generated with: gastly keysorderedbyvalues.go ../intkeysorderedbyintvalues.go holdme NumericKeyType=droptype:int NumericValueType=droptype:int KeysOrderedByValues=IntKeysOrderedByIntValues
...
func NewIntKeysOrderedByIntValues(cap int, randIntn func(int) int) *IntKeysOrderedByIntValues {
	return &IntKeysOrderedByIntValues{
		Keys:     make([]int, 0, cap),
...
func (x *IntKeysOrderedByIntValues) Add(key int, value int) {
	...
	if int(key) == ln {
		...
		x.Values = append(x.Values, make([]int, int(key)-ln+1)...)
		x.Values[key] = value
	}
	right := 0
	hi := len(x.Keys)
